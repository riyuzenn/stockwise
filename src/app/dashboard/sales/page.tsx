...
export default function POSPage() {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setCart([])
        const updatedProducts = await axios.get('/api/product/get')
  ...
  return (
    <div className="grid grid-cols-12 gap-6 p-4 sm:p-6 text-white">
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {paginatedItems.map((item) => (
                <Card
                  key={item._id}
                  className="bg-neutral-800 border-neutral-700 hover:bg-neutral-700 transition"
                >
